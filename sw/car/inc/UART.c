...
// NOTE: This may need to change, I don't remember which UART I wired the ESP up to
void UART_Init(void){
  SYSCTL_RCGCUART_R |= (1 << 5);            // activate UART5
  SYSCTL_RCGCGPIO_R |= 0x10;            // activate port E for UART5
  ...
  UART5_FBRD_R = 26;                    // FBRD = round(0.4028 * 64 ) = 26
	...
	UART5_IM_R |= 0x10; // enable RX interrupts
	
	UART5_IFLS_R &= ~0x38; // RX interrupt on >= 1/8 full
	
	// Enable UART5 interrupt in NVIC
	// Set bit 29 of EN1
  NVIC_EN1_R |= 0x20000000;

  // Set the priority of UART5 interrupt to 1
	// UART5 NVIC is 61, so PRI15's [15:13] bits.
  NVIC_PRI15_R &= ~(0xFFFFDFFF);
	
  UART5_LCRH_R = (UART_LCRH_WLEN_8|UART_LCRH_FEN);
	UART5_CTL_R = 0x0201; // pulled from 319 - enable uart5, only enable receive
  ...
  GPIO_PORTE_DEN_R |= 0x30;             // enable digital I/O on PE5-4
	...
	// Configure PCTL PCM5 and PCM4 for UART. Page 1351.
	// Value of 1 for PCM5/4 configures for UART
  GPIO_PORTE_PCTL_R = (GPIO_PORTE_PCTL_R&0xFF00FFFF)+0x110000;
	
  GPIO_PORTE_AMSEL_R &= ~0x03;          // disable analog functionality on PA
}
